...
#define DoOut(lo,hi,v)  Z80_Out( (word) ((lo)|((word) ((hi)<<8) ) ),v)
...
INLINE void M_WRMEM_WORD (dword A,dword V)
{
 M_WRMEM (A,V&255);
...
// if (R.BC.D && !(R.AF.B.l&Z_FLAG)) { Z80_ICount-=5; R.PC.W.l-=2; }
 if (R.BC.D && !(R.AF.B.l&Z_FLAG)) { M_CSTATE(5); R.PC.W.l-=2; }
}
...
{
 ind ();
 if (R.BC.B.h) { M_CSTATE(5); R.PC.W.l-=2; }
}
...
{
 ldd ();
 if (R.BC.D) { M_CSTATE(5); R.PC.W.l-=2; }
}
